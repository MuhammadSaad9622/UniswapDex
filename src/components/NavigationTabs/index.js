...
document.addEventListener('DOMContentLoaded', function () {
  const fiatTab = document.getElementById('tab-fiat'); // Ensure the element exists

  if (fiatTab) {
    fiatTab.addEventListener('click', () => {
      handleTabClick('/Fiat'); // Trigger the click handler for the Fiat tab
    });
  } else {
    console.error('Element #tab-fiat not found');
  }
});
...
function handleTabClick(path) {
  const selectedTab = tabOrder.find(tab => tab.regex.test(path));

  if (selectedTab && selectedTab.action) {
    selectedTab.action(); // Only trigger the redirect when user clicks
  } else {
    // Handle rendering or navigating to the path normally
